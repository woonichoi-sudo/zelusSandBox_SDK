...
> ⚠️ **세션·워커·로드 횟수를 재는 실험은 `localhost:3000` 에서 하라** (게이트웨이가
> `frontend/dist` 를 직접 서빙한다. 먼저 `cd frontend && npm run build`).
> Vite 를 거치면 **게이트웨이를 재시작할 때마다 세션이 하나 더 생긴다** — 워커도
> 하나 더 뜨고 씬도 한 번 더 로드된다. 프록시의 동작이고 제품 경로에는 없다.
> 이걸 모르고 ISSUE-025 가 "제품 결함" 으로 등재됐다가 닫혔다.
