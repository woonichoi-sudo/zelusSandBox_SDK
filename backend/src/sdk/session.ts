...
export class Session extends EventEmitter {
  ...
  /**
   * 옷 위 그래픽(로고) 목록 (LG-1). 씬을 안 바꾼다.
   *
   * ★ 로고 메시는 좌표가 아니라 **패턴 메시 기준 무게중심**으로 정의돼 있어서,
   *   옷이 움직여도 다시 받을 필요가 없다 — 받는 쪽이 같은 식으로 계산한다.
   * ⚠️ `showIn3DView` 가 꺼진 로고도 목록에는 온다. 그리지 않는 판단은 화면의
   *    일이다 — 여기서 걸러 내면 "왜 안 보이는지" 를 화면이 말할 수 없다.
   */
  logos(): Promise<Record<string, unknown>> {
    return this.#call('logos');
  }
