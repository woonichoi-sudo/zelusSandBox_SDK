 ...
 * 언제든 되돌릴 수 있게 했다.
 *
 * 기본값은 **영어**다 (2026-08-27 에 한국어에서 바꿨다 — 화면을 처음 여는
 *    사람이 영어를 본다). 다만 스모크 831건 중 여럿이 화면 글자를 한국어로
 *    단언한다(`playLabel` 이 '재생' 인지, 커플링 라벨이 '없음' 인지 …).
 *    그래서 스모크는 `main()` 첫머리에서 `setLang('ko')` 로 한국어를 못박고
 *    시작하고, `verify:ui` 는 페이지가 뜨기 전에 저장소에 'ko' 를 넣는다.
 *
 * ⚠️ 한국어 사전의 값이 **예전 하드코딩과 한 글자도 다르지 않아야** 그 단언이
